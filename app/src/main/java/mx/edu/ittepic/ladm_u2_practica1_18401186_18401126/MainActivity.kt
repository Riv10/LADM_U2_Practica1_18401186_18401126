package mx.edu.ittepic.ladm_u2_practica1_18401186_18401126
        ...
        while(true){
            runOnUiThread{
                if(!pausar){
                    ...
                    imgCartas.setImageResource(VectorCartas[VectorBarajas[contador++]])
                    audio?.start()

                    if(contador==VectorBarajas.size)
                        terminar=true

                    if(terminar==true){
                        txtLoteria1.text = "La loteria se ha acabado"
                    }

                }else if(pausar==true){
                            txtLoteria1.text = "Tenemos un ganador"
                      }else if(pausar==false && continuar==true){
                                txtLoteria1.text = "Cartas faltantes"
                                audio = MediaPlayer.create(puntero,VectorAudios[VectorBarajas[contador]])
                                imgCartas.setImageResource(VectorCartas[VectorBarajas[contador++]])
                                audio?.start()
